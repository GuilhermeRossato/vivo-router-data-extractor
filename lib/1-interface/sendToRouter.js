...
module.exports = async function sendToRouter(url, {
    ...
    try {
        ...
        duration = new Date().getTime() - startDate.getTime();
        console.log('Request', requestCount, 'took', Math.floor(duration), 'ms to:', method, url);
        if (lastRequestEndTime) {
            console.log(new Date().getTime() - lastRequestEndTime, 'ms since previous request finished');
        }
