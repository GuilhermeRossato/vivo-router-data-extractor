//@ts-check
const debug_amount_of_html_files_to_keep = 0;
...
async function updateDebugFileIfNecessary(startDate, duration, method, url, sessionId, requestHeaders, responseStatus, responseHeaders, responseBody) {
    ...
    await fs.promises.writeFile(`./exchange-${(requestCount)}.html`, `${header}\n${responseBody ? responseBody : ''}`, 'utf-8');
}
...
module.exports = async function sendToRouter(url, {
    ...
    try {
        // console.log(`Sending request #${requestCount}:`, method, url);
        if (requestCount !== 0) {
            // console.log('Previous request started', (new Date().getTime() - lastRequestStartTime), 'ms ago and', ...(!lastRequestEndTime ? ['there is no finish date'] : ['finished', new Date().getTime() - lastRequestEndTime, 'ms ago']));
        }
        requestCount++;
        if (debug_amount_of_html_files_to_keep && requestCount < debug_amount_of_html_files_to_keep) {
            console.log(`Saving request and response data to "./request-${requestCount}.html" (${debug_amount_of_html_files_to_keep - requestCount - 1} remaining)`);
        }
        ...
        const hasRedirectToRoot = lines.length <= 10 && lines.find(l => l.includes('language=\'javascript\'>\nwindow.top.location = "/";\n</script>'));
