...
module.exports = async function sendToRouter(url, {
    ...
    try {
        ...
        duration = new Date().getTime() - startDate.getTime();
        console.log('Request', requestCount, 'resolved after', Math.floor(duration), 'ms to', method, url, lastRequestEndTime ? new Date().getTime() - lastRequestEndTime : '[no data]', 'ms since previous finished');
