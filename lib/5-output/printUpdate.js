...
const hasDate = process.argv.includes('--date');
...
module.exports = function printUpdate({key, created, updated, removed, previous, value}) {
    ...
    if (isJson) {
        hasDate && (parts.date = date);
        hasTime && (parts.time = time);
        parts.type = created ? 'created' : updated ? 'updated' : removed ? 'removed' : 'unknown';
        parts.key = key;
        hasPrevious && (parts.previous = previous);
        parts.value = value;
    } else {
        hasDate && (parts.push(`${date} `));
