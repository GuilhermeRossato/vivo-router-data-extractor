const { filterIncludes, filterExcludes, isSlowMode, isJson, removeDate, hasTime, hasType, hasPrevious, extractionPeriod } = require("../0-primitive/args.js");
...
if (filterIncludes.length) {
    console.log('Initialized variable filtering with', filterIncludes.length, 'inclusions');
}

if (filterExcludes.length) {
    console.log('Initialized variable filtering with', filterExcludes.length, 'exclusions');
}
...
async function schedulePrint(text) {
    if (isNaN(slow.periodMs)) {
        const period = parseInt(extractionPeriod.toString().replace(/\_/g, ''));
...
module.exports = function printUpdate({key, created, updated, removed, previous, value}) {
    ...
    if (filterIncludes || filterExcludes) {
        ...
        if (filterExcludes?.length && filterExcludes.some(f => simple.startsWith(f))) {
            console.log('Filtering', key, 'variable because it matches a exclusion argument');
            filterRecord[key] = true;
            return;
        }
    }
