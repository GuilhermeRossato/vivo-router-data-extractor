...
/**
 * @returns {{ROUTER_USERNAME: string, ROUTER_PASSWORD: string, ROUTER_HOST: string, ROUTER_EXTRACT_PERIOD_MS: string}}
 */
function applyDefaultEnv(env) {
    ...
    if (!env.ROUTER_EXTRACT_PERIOD_MS) { env.ROUTER_EXTRACT_PERIOD_MS = '20_000'; }
...
module.exports = async function loadEnv() {
    if (!cachedEnv) {
        ...
        for (const line of envFileLines) {
            ...
            env[line.substring(0, equal).trim()] = line.substring(equal + 1).trim();
        }
