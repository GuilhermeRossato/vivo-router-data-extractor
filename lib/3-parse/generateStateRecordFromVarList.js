...
const { includeOriginalValue } = require("../0-primitive/args.js");
...
module.exports = function generateStateRecordFromVarList(varList) {
    ...
    const state = {};
    for (const {name, content, original} of varList) {
        ...
        if (includeOriginalValue && keys.length !== count) {
            const key = keys.pop();
            if (typeof state[key] === 'string') {
                state[key] = `${state[key]} (Original: ${original})`;
            } else if (state[key] && typeof state[key] === 'object') {
                state[key].original = original;
            } else {
                state[`${key}Original`] = original;
            }
        }
