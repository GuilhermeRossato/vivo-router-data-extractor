const { isOnlyStatus, includeIndividualHost } = require("../0-primitive/args.js");
    ...
    for (const key in newState) {
        ...
        if (typeof newState[key] === 'object') {
            for (const prop in newState[key]) {
                ...
                if (cachedHostRecord[mac][prop] instanceof Array) {
                ...
                if (typeof cachedHostRecord[mac][prop] === 'string') {
            ...
            if (name === 'ipv6MacList') {
                name = 'ipv6';
            } else if (name === 'portMacList') {
                name = 'port';
            }
            cachedHostRecord[mac][name] = newState[key];
        }
        if (!includeIndividualHost) {
            delete newState[key];
