...
module.exports = async function startExtractionLoop(
    ...
    while (true) {
        ...
        console.log('State', newVarList.length === 0 ? 'not augmented (no keys updated)' : `augmented in ${newVarList.length} keys`);
        const updateList = getUpdateListFromStateRecordPair(oldState, oldTime, newState, response.time);
        if (updateList.length) {
            console.log(updateList.length === 1 ? 'There is' : 'There are', updateList.length, updateList.length === 1 ? `update to apply at key` : 'updates to apply starting with', `"${updateList[0].key}"`);
